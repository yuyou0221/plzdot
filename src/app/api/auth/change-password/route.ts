...
import { isUserDataAuditWriteError, recordUserDataAuditLog, type UserDataAuditWriteError } from "@/lib/user-data-audit";

export const runtime = "nodejs";

const maxPasswordLength = 128;

function auditWriteFailureResponse(error: UserDataAuditWriteError) {
...
export async function POST(request: Request) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (isUserDataAuditWriteError(error)) {
      return auditWriteFailureResponse(error);
    }
    throw error;
  }
}
