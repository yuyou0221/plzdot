...
- 红蜡路线 A：#17 红蜡确认为 21 个工作日，等待 #14 / #15。
- 手板路线 B：#17 红蜡确认 / 手板确认为 10 个工作日，等待 #18。
- 两条路线下 #21 T1 灰模都仍等待 #17 / #12。
- 该规则由内核任务图统一维护，Excel 和页面不得临时覆盖。
- 实际进度套入。
- 后置任务推断完成。
- 预测完成日期。
- 延期、风险、阻塞上线判断。
...
- 虚拟款式不能导入。
- 虚拟款式不能生成项目排期完成事实。
- 所有任务 7 / 10 必做真实款式已通过后，建模排期只提供项目排期可读取的建模完成事实。
- 是否写项目任务执行事实、是否重新测算，由项目排期模块自行决定。
