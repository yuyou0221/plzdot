...
project-main-full-refresh：项目主数据 Excel，全量更新模式，仅最高权限账号可用。
```
...
8. `project-main-full-refresh` 会额外返回“将移出规划”的旧项目清单；这些项目是数据库中存在但 Excel 中不存在的项目。
```
...
project-main-full-refresh：项目主数据 Excel，全量更新模式。
```
...
11. 导入完成后会自动触发正式排期重算；如果重算失败，会返回“数据已保存但重算失败”的错误信息。
12. `project-main-full-refresh` 只允许最高权限账号使用；确认导入后，Excel 中不存在的旧项目会被标记为“已移出规划”，并清理其正式排期读模型，使其不再出现在项目排期、正式测算和项目看板中。
13. 全量更新不会物理删除 Project，不会删除建模、财务、审计等历史业务事实。
```
...
理论盈亏开发成本 = 实际开发成本；未录入实际开发成本时使用理论开发成本
理论盈亏 = 理论测算销量 × 零售价 × 折扣 - 理论盈亏开发成本 - 渠道样品成本 - 展示盒成本 - 理论测算销量 × 实际测算用单件生产成本
